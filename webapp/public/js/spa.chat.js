...
import {spautil} from './spa.util.js';
import {spautil_b} from './spa.util_b.js';
import {chat_mustache} from './chat/chat.mustache.js';
import './jq/jquery.event.gevent.js';

export var spachat = (function () {
  ...
  //inizializzo proprietà della chat
  ...
  setPxSizes = function () {
    ...
    px_per_em = spautil_b.getEmSize( jqueryMap.$slider.get(0) );
  ...
  writeChat = function ( person_name, text, is_user ) {
    var msg_class = is_user
      ...
      jqueryMap.$msg_log.append(
        '<div class="' + msg_class + '">'
        + spautil_b.encodeHtml(person_name) + ': '
        + spautil_b.encodeHtml(text) + '</div>'
      );
      scrollChat();
  };
  writeAlert = function ( alert_text ) {
    jqueryMap.$msg_log.append(
      ...
      + spautil_b.encodeHtml(alert_text)
      + '</div>'
    );
    scrollChat();
  };
  ...
  onListchange = function ( event ) {
    ...
    people_db().each( function ( person, idx ) {
      ...
      + spautil_b.encodeHtml( person.name ) + '</div>';
    });
...
// End public method /removeSlider/
  ...
  initModule = function ( $append_target ) {
	...
	chat_mustache.callMustache($append_target, configMap);
    setJqueryMap();
    setPxSizes();
