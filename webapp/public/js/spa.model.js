...
import {spafake} from './spa.fake.js';

export var spamodel = (function () {
  'use strict';
  ...
  makePerson, removePerson, chat, people, initModule;

  //proprietà dell'oggeto person esposte
  personProto = {
  ...
  completeLogin = function ( user_list ) {
    ...
    chat.join();
    // When we add chat, we should join here(page 155)
    $.gevent.publish( 'spa-login', [ stateMap.user ] );
  };
  //Costruttore dell'ogetto person.
  //Costruisce person e lo salva nel db.
  ...
  var
    ...
    _update_list = function( arg_list ) {
      ...
      for ( i = 0; i < people_list.length; i++ ) {
        ...
        //se esiste il chatee ed è presente in lista (id!=null)
    ...
    _publish_updatechat = function ( arg_list ) {
      ...
      //lancia evento che ha aggiornato la chat
      $.gevent.publish( 'spa-updatechat', [ msg_map ] );
    };
    // End internal methods
    _leave_chat = function () {
      var sio = isFakeData ? spafake.mockSio : spa.data.getSio();
      chatee = null;
    ...
    join_chat = function () {
      ...
      sio = isFakeData ? spafake.mockSio : spa.data.getSio();
  ...
  people = (function () {
    ...
    login = function ( name ) {
        //somula chiamata socket al beckend
        var sio = isFakeData ? spafake.mockSio : spa.data.getSio();
        ...
        //registra la callback a fine login
        sio.on( 'userupdate', completeLogin );
        //manda un messagio al beckend fittizio con i dati del login
        //sarà il fake a lanciare la callback completelogin
        sio.emit( 'adduser', {
  ...
  initModule = function () {
    ...
    /*
    if ( isFakeData ) {
      people_list = spafake.getPeopleList();
      for ( i = 0; i < people_list.length; i++ ) {
        person_map = people_list[ i ];
        makePerson({
          cid : person_map._id,
